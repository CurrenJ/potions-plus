...
| 11 | Client (renderers, particles, tooltips, colors, models, JEI ×3) | 🟡 **partial, Phase 11a steps 1-2 of 4 closed 2026-09-04 (second session)** — particles, key mappings, item-tint color, and JEI still done and smoke-verified on all three loaders. **Phase 11a step 1 (common `BlockEntityType` hub) and step 2 (packet moves) landed**: `common/core/Blocks.java` now holds all 6 `BlockEntityType` holders (4 as `Holder<BlockEntityType<?>>` pending steps 3/4, 2 concretely typed); `ClotheslineBlockEntity`/`PotionBeaconBlockEntity` (+`ClotheslineBlockEntityBakedRenderData` + both renderers) and their `Block` classes moved `neoforge/` → `common/`; Clothesline/PotionBeacon now register as real blocks+BE types+renderers on **all three loaders** (not just NeoForge) — Fabric via `FabricBlockEntityTypeBuilder`/`BlockEntityRendererRegistry`, Forge via `ForgeHolder`-wrapped `BlockEntityType.Builder.of(...).build(null)` + `EntityRenderersEvent.RegisterRenderers`; `ClientboundAcquiredBrewingRecipeKnowledgePacket` moved to `common/network/` and wired on all three `Packets.java`. **Steps 3 (`DynamicIconItems` DSL) and 4 (`RecipesRegistrar`) remain, out of scope this session** — `BrewingCauldronBlockEntity`/`HerbalistsLecternBlockEntity`/`SanguineAltarBlockEntity`/`AbyssalTroveBlockEntity` still neoforge-only, block (cauldron water) tint and the `ItemStacksTooltip` factory still blocked. **New blocker found and correctly left alone, not forced**: the 2 `ClientboundSanguineAltarConversion{State,Progress}Packet`s are NOT Phase-5-shaped moves as previously assumed — they reference the concrete `SanguineAltarBlockEntity` class directly (its `State` enum, public fields), not just its `BlockEntityType` holder, so the step-1 hub does not unblock them; still neoforge-only. See the 2026-09-04-b progress-log entry for full evidence. |
...
**Recommendation for whoever picks this up next** (updated 2026-09-04, Phase 11a steps 1-2 now
**closed**, see the 2026-09-04-b progress-log entry): the 4-step order below still holds for steps 3-4.
~~(1) build the common `BlockEntityType` hub + fabric/forge registration (Phase-4-shaped, unblocks
`PotionBeaconBlockEntity`/`ClotheslineBlockEntity` immediately, and their Block classes)~~ **done** —
`common/core/Blocks.java` exists, both BE classes + Block classes + renderers are common and registered
on all three loaders. ~~(2) move the 3 remaining packets (Phase-5-shaped, unblocks
`PlayerBrewingKnowledgeNetworking`/`SanguineAltarBlockEntity`)~~ **partially done** —
`ClientboundAcquiredBrewingRecipeKnowledgePacket` moved (unblocked `PlayerBrewingKnowledgeNetworking`
for real); the 2 `ClientboundSanguineAltarConversion{State,Progress}Packet`s turned out to reference
the concrete `SanguineAltarBlockEntity` class directly (not just its `BlockEntityType` holder) and are
still neoforge-only — moving them needs either porting `SanguineAltarBlockEntity` itself or a new
common interface abstraction, neither of which is a Phase-5-shaped mechanical move; revisit alongside
step 3, since `SanguineAltarBlockEntity` needs `DynamicIconItems` anyway. (3) decide whether to port or
re-abstract the `DynamicIconItems` item-builder DSL (the single largest remaining piece, blocks
`AbyssalTroveBlockEntity`/`BrewingCauldronBlockEntity`/`HerbalistsLecternBlockEntity`, and now also the
2 Sanguine Altar packets above); (4) revisit `RecipesRegistrar`'s runtime-injection half only after
Phase 9's access widener lands. Only after (3)-(4) does the rest of the original Phase 11 client work
(remaining 4 BE renderers, block tint, tooltip factory) described in the task brief become unblocked.
...
environment to join a world and place a block). The other 4 BE renderers, the block (cauldron water)
tint, and the tooltip component factory remain neoforge-only pending Phase 11a steps 3-4.
