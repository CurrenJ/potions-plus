...
| 11 | Client (renderers, particles, tooltips, colors, models, JEI ×3) | 🟡 **partial, blocker re-scoped (deeper) 2026-09-04** — particles, key mappings, item-tint color, and JEI still done and smoke-verified on all three loaders (unchanged from 2026-09-03). One small real port landed: `HerbalistsLecternSounds` moved to `common/blockentity/` (it had zero remaining neoforge coupling). **BE renderers, block (cauldron water) tint, and the `ItemStacksTooltip` component factory remain blocked, and the blocker is substantially larger than the 2026-09-03 note described**: it is not just "6 BE classes + `DynamicIconItems`" but an entire un-split registration layer (BlockEntityType hub, item-builder DSL, recipe-injection registrar, 2 Block classes, 3 packets) — see the blocker note below and the 2026-09-04 progress-log entry for the full per-class evidence. This is Phase-4/5/8-sized work, not a quick unblock, and was not attempted this session beyond the one safe file move. |
...
**Recommendation for whoever picks this up next**: treat this as its own scoped sub-phase (call it
Phase 11a) with this order: (1) build the common `BlockEntityType` hub + fabric/forge registration
(Phase-4-shaped, unblocks `PotionBeaconBlockEntity`/`ClotheslineBlockEntity` immediately, and their
Block classes); (2) move the 3 remaining packets (Phase-5-shaped, unblocks
`PlayerBrewingKnowledgeNetworking`/`SanguineAltarBlockEntity`); (3) decide whether to port or
re-abstract the `DynamicIconItems` item-builder DSL (the single largest remaining piece, blocks
`AbyssalTroveBlockEntity`/`BrewingCauldronBlockEntity`/`HerbalistsLecternBlockEntity`); (4) revisit
`RecipesRegistrar`'s runtime-injection half only after Phase 9's access widener lands. Only after (1)-(4)
does the actual Phase 11 client work (renderers, tint, tooltip factory) described in the task brief
become unblocked.
