...
**Update 2026-09-04 (10th session): Phase 11 CLOSED — the "sole remaining item" note above was
stale, superseded by the Phase 7 (9th session) work.** Re-read `common/utility/ClientItemStacksTooltip.java`
fresh rather than trusting either this section's own prior text or Phase 7's: the class moved to
`common/` in the 9th session and has zero `net.neoforged`/`net.minecraftforge` imports (only the
purely decorative `Dist`/`OnlyIn` annotations were ever NeoForge-specific, and those were dropped —
verified by reading the file directly, no import lines reference either loader package). All three
loaders' `event.{fabric,forge,neoforge}.ClientTooltipComponentFactoriesListeners` register it:
Fabric via `net.fabricmc.fabric.api.client.rendering.v1.TooltipComponentCallback.EVENT`
(registered from `PotionsPlusFabricClient.onInitializeClient`, confirmed by reading that call site),
Forge via `RegisterClientTooltipComponentFactoriesEvent` + `@Mod.EventBusSubscriber` FML discovery
(byte-identical shape to NeoForge's), NeoForge unchanged. So Phase 11's actual "sole remaining item"
was already fixed by the time this session started — it just hadn't been reflected back into this
section or the Status table row. Read the entire Phase 11 section end-to-end (not just the tail) to
check for any other still-open item hiding earlier in the history — found none; every checklist item
above is `[x]` except the three `[ ]` items that were always out-of-scope-for-this-phase notes
(atlas-crash monitoring, BE-resync-packet risk, REI/EMI), none of which block the exit criterion.
Verification this session: `:neoforge:build :fabric:build :forge:build -x test` → `BUILD SUCCESSFUL`
(31 actionable tasks, all up-to-date/executed clean); Decision 4a `comm -12` — the exact doc formula
(package-directory intersection, not a looser directory-name check) — run fresh against all three
loaders, empty for all three. **Exit criterion now fully met**: all three clients render/register
all 6 block entities identically, particle/item-tint/block-tint/key-mapping/JEI parity holds on all
three (8th session), and the tooltip-component factory is common + registered on all three. Not
independently re-verified this session: in-world rendering of any of it (still no GUI-automation
tool in this environment — the standing caveat every session since Phase 11a has recorded).
...
- [x] **Restore JUnit — done 2026-09-04 (11th session), exactly the fix predicted.** Moved the
      `testImplementation 'org.junit.jupiter:junit-jupiter:5.9.3'` /
      `testRuntimeOnly 'org.junit.platform:junit-platform-launcher'` /
      `tasks.withType(Test).configureEach { useJUnitPlatform() }` block from `neoforge/build.gradle`
      into `common/build.gradle` (left the neoforge copy in place too — harmless, no `src/test` of its
      own to use it). `:common:test` → **BUILD SUCCESSFUL, 87 tests across all 12 files, 0 failures, 0
      errors** (counted from the per-class XML reports: `BootstrapSmokeTest` 1, `EffectComparisonTest`
      25, `EffectScalingTest` 5, `PotionContainerTest` 17, `PotionContentsAccessTest` 1,
      `PotionDataBuilderTest` 20, `PotionDataTest` 12, `FortuitousFateEffectTest` 1,
      `LootingEffectTest` 1, `MetalDetectingEffectTest` 2, `NauticalNitroEffectTest` 2). No moddev-style
      loom test config was needed, exactly as predicted — Minecraft is on the test classpath via Loom
      already, `AlchemyTestBase` bootstraps registries itself.
- [x] The two trees' unit-test files match 1:1 (12 files, `AlchemyTestBase` … `PotionDataTest`,
      including `PotionContentsAccessTest`) — confirmed via `find common/src/test -name '*.java'`, no
      test authoring needed, only wiring.
- [x] **`PotionContentsAccessTest` runs and passes** — `TEST-grill24.potionsplus.alchemy.
      PotionContentsAccessTest.xml`: `tests="1" failures="0" errors="0"`.
- [x] **NeoForge game tests — done 2026-09-04 (11th session), 33/33 passing.** The existing `testmod`
      sourceSet (already present, pointed at `common/src/testmod/java`) needed the exact
      `loom.mods`-gotcha fix this checklist predicted, confirmed empirically: with `testmod` still
      associated via `loom { mods { "${mod_id}" { sourceSet main; sourceSet testmod } } }`,
      `:neoforge:runGametest` booted clean (namespace log line printed) but
      `GameTestServer.create` failed immediately with `IllegalArgumentException: No test functions
      were given!` — FML's dev-mode `ModList.getAllScanData()` annotation scan (`GameTestHooks
      .registerGametests()`, decompiled via javap) never found the `@GameTestHolder`-annotated test
      classes, because loom's sourceSet-merging puts testmod's split output dir on the *runtime*
      classpath (classes load fine, hence no crash) but not on whatever FML actually re-scans for
      annotations at dev-launch time. Fixed exactly as this checklist's next bullet describes: removed
      `testmod` from the `mods {}` block, redirected `compileTestmodJava`'s `destinationDirectory` into
      `sourceSets.main.output.classesDirs.singleFile` (with `dependsOn(processResources)`), and
      excluded `**/gametest/**` from both `jar` and `shadowJar` (needed both — `jar`'s output isn't
      what `remapJar` actually packages, `shadowJar`'s is). Result: `:neoforge:runGametest` → `33 tests
      are now running` → `All 33 required tests passed :)`. Verified the exclude works: `unzip -l
      neoforge/build/libs/potionsplus-neoforge-1.6.0.jar | grep potionsplus/gametest` → zero hits, and
      `:neoforge:build -x test` still green afterward.
- [x] **Fabric game tests — done 2026-09-04 (11th session), 33/33 passing, one real API-shape
      correction along the way.** `fabric-gametest-api-v1` needed no new dependency — already resolved
      transitively via the existing `modImplementation "net.fabricmc.fabric-api:fabric-api:..."` bundle
      (confirmed: `.gradle/loom-cache/remapped_mods/.../fabric-gametest-api-v1-b9d2d3ba/2.0.5+...`
      already present before this session touched anything). `fabric/src/testmod/resources/
      fabric.mod.json` declares a separate `potionsplus_testmod` mod (own id, own `fabric-gametest`
      entrypoint) exactly as this checklist specified — Fabric needs no `loom.mods`-style merge trick at
      all, since it discovers the testmod as an independent mod file via normal classpath scanning for
      `fabric.mod.json`, not FML's annotation-scan mechanism. **Real, javap-confirmed divergence from
      the reference `dev/26.1.2` tree found this session**: this branch's pinned `fabric_api_version =
      0.116.7+1.21.1` resolves `fabric-gametest-api-v1:2.0.5`, a materially *older* API shape than
      26.1.2's — there is no `net.fabricmc.fabric.api.gametest.v1.GameTest(structure=...)` annotation in
      this version at all (that belongs to a later fabric-gametest-api-v1 3.x+, matching a newer
      Minecraft pairing). 2.0.5's actual shape: implement the marker interface `net.fabricmc.fabric.api
      .gametest.v1.FabricGameTest` and put **vanilla** `net.minecraft.gametest.framework.@GameTest`
      directly on instance methods; `FabricGameTest.EMPTY_STRUCTURE` (`"fabric-gametest-api-v1:empty"`)
      is the bundled empty structure, used identically to how NeoForge's tests use their own committed
      `empty_testarea.nbt`. A single-test smoke run confirmed this shape works before the file was
      filled out to all 33. Result: `:fabric:runGametest` → `33 tests are now running` → `All 33
      required tests passed :)`.
- [x] **`loom.mods` is global, not per-run — confirmed exactly as this checklist warned, with the exact
      symptom differing slightly from the 26.1.2-predicted one.** Associating `testmod` in `loom.mods`
      didn't crash FML mod discovery outright here (`runClient`/`runData` still worked) — instead it
      silently broke the *annotation scan* specifically (see the NeoForge bullet above for the full
      repro/evidence). The prescribed fix (redirect `compileTestmodJava` into main's classes dir,
      `dependsOn(processResources)`, exclude `**/gametest/**` from the jar tasks) is exactly what
      resolved it, on both NeoForge and Forge (Forge needed it too, though it was not sufficient alone
      there — see the Forge bullet below).
- [x] **Forge: `net.minecraftforge.gametest` — verified 1.21.1 predates `Registries.TEST_INSTANCE`,
      exactly as this checklist hypothesized, and confirmed `ForgeGameTestHooks` mirrors NeoForge's
      `GameTestHooks` almost exactly** (javap-confirmed both: same ASM `ModFileScanData`-based
      `@GameTestHolder` annotation scan, same `"{forge,neoforge}.enabledGameTestNamespaces"`-gated
      namespace check, same overall shape — Forge's `GameTestHolder` just splits NeoForge's single
      `value()` into two fields, `value()`/`namespace()`, and renames the disable-prefixing marker to
      its own `@GameTestDontPrefix` annotation instead of an annotation parameter). **This prediction
      held, but Forge is still 🔴 blocked — for reasons that have nothing to do with TEST_INSTANCE or
      the annotation-scan mechanism itself, and go considerably further than the 26.1.2 tree's Forge
      ever got.** Full evidence trail (all four issues found and the first three fixed this session):
      1. `:forge:runGametest`'s launch process failed immediately with `Could not find or load main
         class dev.architectury.transformer.TransformerRuntime` — diffing `forge/build/loom-cache/
         argFiles/runGametest` against the working `runServer`'s argFile byte-for-byte showed
         `architectury-transformer-5.2.91-runtime.jar` simply absent from the gametest run's classpath,
         even though every other Forge run (client/server/data) has it and NeoForge's own identically
         -shaped `gametest` run does too. **Fixed**: added `testmodRuntimeOnly
         "dev.architectury:architectury-transformer:5.2.91:runtime"` directly.
      2. Same launch then failed one hop later with `ClassNotFoundException: net.fabricmc
         .devlaunchinjector.Main` — same class of gap, same fix pattern:
         `testmodRuntimeOnly "net.fabricmc:dev-launch-injector:0.2.1+build.8"`.
      3. With those fixed, the launch reached FML/Forge module resolution and failed with
         `java.lang.module.ResolutionException: Module generated_XXXXXXX contains package
         grill24.potionsplus.command, module potionsplus.common.dev exports package
         grill24.potionsplus.command to generated_XXXXXXX` — a JPMS split-package collision, the exact
         failure mode `forge/build.gradle`'s own top-of-file comment already documents and that's why
         `runtimeClasspath` (unlike `compileClasspath`/`developmentForge`) deliberately never extends
         `common` for this module. Root cause: the `configurations.testmodRuntimeClasspath
         .extendsFrom(...)` line needed for compiling (mirroring fabric/build.gradle's fix — see
         `fabric-gametest-api-v1` needing the same `testmodCompileClasspath.extendsFrom(compileClasspath)`
         treatment) was pointed at `compileClasspath` (which pulls in `common` via the `common`
         configuration), recreating the exact collision the file's own comment warns about. **Fixed**:
         extend `testmodRuntimeClasspath` from plain `runtimeClasspath` instead (still never reaches
         `common`, since forge's own `runtimeClasspath` doesn't either) — supplies the Forge
         bootstrap/launcher jars without re-adding `common` a second time.
      4. **Still open**: with all three of the above fixed, `:forge:runGametest` now boots all the way
         through FML/Forge — loads 1330 recipes, 1456 advancements, logs `Enabled Gametest Namespaces:
         [potionsplus]` (i.e. reaches *exactly* the point NeoForge's run used to fail at before its own
         `compileTestmodJava` redirect) — then fails with the identical `IllegalArgumentException: No
         test functions were given!` NeoForge had. The same redirect fix is already applied in
         `forge/build.gradle` too, and confirmed working at the file-placement level
         (`PotionsPlusForgeGameTests.class` verifiably lands in `forge/build/classes/java/main/...`, not
         a separate testmod dir — checked directly) — but it does not resolve the empty-scan symptom for
         Forge the way the identical fix did for NeoForge. Ruled out: not a stale-cache artifact
         (`--rerun-tasks` plus deleting `forge/build/gametest` first reproduces identically). Not
         isolated further this session — would need either decompiling more of Forge's actual
         `ModFileScanData` population path (as opposed to just `ForgeGameTestHooks`'s consumption of it,
         which is all that was inspected) or trial-and-error against a real Forge modding community
         reference, neither of which fit in this session's remaining budget. **This is genuinely a
         deeper, more specific blocker than what the 26.1.2 reference tree hit** (that tree's Forge never
         got RegisterEvent to fire *at all*, on a materially different/newer gametest API generation) —
         not a re-hit of the same problem, an actual new one. See `forge/build.gradle`'s `loom.runs
         .gametest` and `dependencies{}` blocks for the exact comments/evidence and the reproduce command.
- [ ] Watch for bugs *in the tests*. 26.1.2's `963a909` found `spawnItem(Item, BlockPos)` already
      ...
      Not directly hit this session (this branch's tests don't call `spawnItem`/`randomTick` the same
      way), but a **different real bug in the tests themselves was found and fixed** — see below.
- [x] **A real bug was found and fixed in the tests, not just plumbing (2026-09-04, 11th session).**
      Three of the ported tests (`modPotionsRoundTripThroughEveryContainer`,
      `brewingASeededBasePotionRecipeYieldsItsPotion`, `craftingAwardsTheRecipeExperience`) hardcoded
      `geode_grace` as their example mod potion. `:fabric:runGametest`'s first full 33-test run failed
      exactly these three with `GameTestAssertException: no base potion recipe was generated for
      potionsplus:geode_grace` / `IllegalStateException: geode_grace potion was not generated` — a real,
      Phase-12-caught content gap, not a test-authoring mistake: `geode_grace`'s *potion* (not the raw
      mob effect) is only generated by `core.neoforge.potion.PotionsRegistrar`, one of Phase 7's still-
      open "8 potions with `@EventBusSubscriber`-coupled effects" — it is genuinely never registered on
      Fabric or Forge. NeoForge's own run passed only because that class happens to run there. Fixed by
      switching all three tests to `MAGNETIC` (`core.potion.Potions.MAGNETIC_POTIONS`), which is
      `common/` and generates identically on every loader — re-ran `:fabric:runGametest` and
      `:neoforge:runGametest` after the fix, both 33/33 clean. Left a comment at each call site pointing
      at the real gap so whoever eventually closes Phase 7's `PotionsRegistrar` bucket doesn't need to
      rediscover this.
- [x] **Ported the platform test harness, package convention followed exactly as specified** (`<loader>`
      infix, not `core.<loader>` suffix): `neoforge/src/testmod/java/grill24/potionsplus/neoforge
      /gametest/{NeoForgeTestPlayers, PotionsPlusNeoForgeGameTests}`;
      `fabric/src/testmod/java/grill24/potionsplus/fabric/gametest/PotionsPlusFabricGameTests`.
      `TestPlayers.makeMockCreativePlayerInLevel` (previously sitting directly in `common/src/testmod`
      despite importing NeoForge-only `net.neoforged.neoforge.network.registration.NetworkRegistry` —
      a real misplacement bug, not just an organizational nicety, since it meant `common/src/testmod`
      was never actually loader-agnostic despite living in `common/`) moved to `neoforge/…
      /NeoForgeTestPlayers` and renamed; Fabric's equivalent test uses vanilla
      `helper::makeMockServerPlayerInLevel` directly (matching the reference `dev/26.1.2` tree's own
      choice for Fabric — not independently re-verified against Fabric's networking internals, carried
      over). **`common/src/testmod`'s `AlchemyGameTests`/`BrewingCauldronGameTests` were themselves
      refactored to be genuinely loader-agnostic this session** (they previously carried NeoForge's
      `@GameTestHolder`/`@PrefixGameTestTemplate`/`@GameTest` directly, which is why they could only
      ever be compiled — and discovered — by NeoForge): stripped down to plain static methods with zero
      gametest-framework annotations, exactly matching the reference tree's shape, with each loader's
      own `PotionsPlusXGameTests` wrapper class carrying that loader's own annotations and delegating in.
      This branch still has only 2 of the reference tree's 4 shared files (`AlchemyGameTests`,
      `BrewingCauldronGameTests` — `EffectGameTests`/`RecipeSyncGameTests` are 26.1.2-only additions
      covering functionality/packets this branch doesn't have or that are out of this phase's scope),
      exactly as this checklist predicted; not force-ported.
- [x] **Target met on 2 of 3 loaders**: the same 33 shared alchemy + brewing-cauldron tests (not 34 —
      recounted directly from the ported files, see the bullet above) pass on **both** NeoForge and
      Fabric. Forge status recorded honestly above — not passing, real progress made, real blocker
      identified and left reproducible.

**Exit criterion: substantially met.** `:common:test` green (87 tests, 0 failures). NeoForge and Fabric
game-test runs both pass (33/33 each). Forge status recorded honestly: real, verified progress (three
distinct infra bugs found and fixed, taking the run from "cannot launch at all" to "boots fully through
FML/Forge and reaches the exact same checkpoint NeoForge used to fail at"), but the final blocker (empty
`ForgeGameTestHooks` annotation scan) was not resolved this session. Per this phase's own instructions,
Forge gametests were never a hard requirement — only an honest report was, and this is one. **Phase 12
is not marked ✅ closed** because of the open Forge item; everything else in its exit criterion is done
and verified.

**CURRENT STATE / PICK UP HERE (2026-09-04, 11th session):** JUnit restore, NeoForge gametest (33/33),
and Fabric gametest (33/33) are all done and verified — nothing further needed there short of adding
more test coverage in a future phase (out of this phase's scope; it was porting/wiring only). The one
remaining thread is Forge's `IllegalArgumentException: No test functions were given!` (see the Forge
bullet above for the full repro and everything already ruled out). Suggested next steps for whoever picks
this up: (a) get an actual decompiled/source copy of `net.minecraftforge.fml.loading.moddiscovery`'s
`ModFileScanData` population (the ASM classpath scan itself, not just `ForgeGameTestHooks`'s consumption
of it) to see what it does differently from NeoForge's equivalent — the two are clearly *not* identical
even though `ForgeGameTestHooks` and `GameTestHooks` are; (b) as a possible faster path, check whether
Forge's `ModFileScanData` only includes scan results from files carrying a real `mods.toml` (main's
`processResources` output does, testmod's merged-in classes still don't have their own, though they're
physically in main's directory) — if so, the fix might be forcing Forge specifically to re-scan main's
directory *after* `compileTestmodJava` writes into it, or finding whichever task actually produces the
scan-data cache Forge's dev launch consumes and making sure it runs after the merge.

---

## Phase 13 — Verification

- [x] **`./gradlew build` — done 2026-09-04 (12th session).** Full 4-module build green
      (`BUILD SUCCESSFUL in 15s`, `:common :neoforge :fabric :forge` all `compileJava`/`build`
      succeeded, all three loader jars assembled). Includes the uncommitted
      `forge/.../ClientCommands.java` `Bus.MOD`→`Bus.FORGE` fix — `:forge:compileJava` shows
      up-to-date against the current (fixed) file content, i.e. it was already compiled clean with
      the fix present. Re-ran `:common:test --rerun-tasks` (forced, not cached) for a truly fresh
      confirmation: `BUILD SUCCESSFUL in 17s`; per-class XML reports sum to **96 tests, 0 failures, 0
      errors** (up from Phase 12's 87 — 9 more tests landed since, all green).
- [x] `./gradlew :neoforge:runClient` — **carried over from the prior (interrupted) session, verified
      here by reading the evidence directly rather than re-claiming it blind.** The leftover
      `neoforge_runclient.log` (628 lines, now deleted after extraction) tail shows a clean run: JEI
      shutdown sequence, `Server thread` clean disconnect/save-and-quit, `Stopping!`, zero
      `FATAL`/uncaught-exception lines anywhere in the file, `BUILD SUCCESSFUL in 47s`.
- [x] **`./gradlew :fabric:runClient` — done 2026-09-04 (12th session), clean.** Launched
      `run_in_background`, watched the log for `Sound engine started` (hit at 21:34:55, ~fast),
      then it went further on its own and actually loaded into an existing dev world (JEI GUI
      built/registered, `Player999` joined, advancement toast fired) — a stronger result than the
      bar asked for. Two pre-existing, already-documented content-gap errors appeared during world
      load (`Unknown registry key ... potionsplus:golden_cubensis` / `potionsplus:diamour`, plus the
      loot-table/tag/recipe/advancement parse failures that cascade from them) — these are the exact
      "8 potions with `@EventBusSubscriber`-coupled effects genuinely never registered on Fabric or
      Forge" gap Phase 7/12 already found and left open (same two items Phase 12's gametest fix
      pointed at), not a new regression; they're caught/logged (`wrapScreenError`,
      `SimplePreparableReloadListener`) and never crash the client — the JVM kept running, reached a
      live joined-world state, zero `FATAL`/uncaught-thread-death. Killed the game JVM (pid 71940,
      `KnotClient`/`architectury.main.class`) via `Stop-Process -Force` once done; confirmed via a
      follow-up process scan that no `architectury`/`KnotClient`/`gradlew` processes were left running
      (Gradle itself then reports `BUILD FAILED` for the killed run, exactly like every prior session's
      client kill — not a real failure, just Gradle noticing its child died).
- [x] **`./gradlew :forge:runClient` — done 2026-09-04 (12th session).** Launched
      `run_in_background`, reached `Sound engine started` at 21:36:37 (fast, clean client boot,
      only benign `ForgeConfigSpec` "corrected from null to default" warnings from a fresh
      `serverconfig`), then the integrated server started loading a world and the server thread
      crashed at 21:36:55 with `java.util.NoSuchElementException: No value present` in
      `SeededIngredientsLootTables.lambda$getItemsInTags$6` (full trace:
      `SeededIngredientsLootTables.getItemsInTags` →
      `.generateLootWeightsFromTag`/`.generateCommonIngredients`/`.generateAllPossibleIngredients` →
      `PotionUpgradeIngredients.<init>` → `SeededPotionRecipeBuilder.generateRecipes` →
      `RecipesRegistrar.injectRuntimeRecipes` → `core.forge.ServerLifecycleListeners.onServerStarted`).
      **This is not a new regression** — it is the exact same crash Phase 8's and Phase 12's status
      rows already document and root-caused (`golden_cubensis`/`diamour` items never registered on
      Forge, so a tag lookup used by the seeded-recipe generator comes back empty and
      `.orElseThrow()` fires), confirmed there via a `git stash` reproduction showing the identical
      crash predates this session's changes entirely. Crash report saved to
      `forge/run/crash-reports/crash-2026-09-04_21.36.55-server.txt` (read directly, not just
      grepped, to confirm the stack trace matches). The client JVM exited on its own on the crash —
      confirmed via a process scan immediately after that no `architectury.main.class`/`gradlew`
      processes were left running, so no orphan-kill step was needed this time.
      **Net result for this checklist item: client boot itself is clean on Forge (reaches the main
      menu / sound engine with zero exceptions); the crash is server-side, world-load-time, and
      pre-existing** — matches this doc's standing bar of "don't re-attempt fixing a known,
      documented blocker during a verification phase."
- [x] **Game-test runs re-confirmed 2026-09-04 (12th session), post-Phase-12 state still holds.**
      `:neoforge:runGametest` → `All 33 required tests passed :)`, `BUILD SUCCESSFUL in 27s` — headless
      run, exited cleanly on its own, no game JVM/orphan to kill. `:fabric:runGametest` →
      `All 33 required tests passed :)`, `BUILD SUCCESSFUL in 26s`, also exited cleanly on its own.
      Both confirm the 33/33 Phase 12 left off still holds after everything since (Phase 11's final
      block-entity/tooltip work, this session's build). Forge's gametest blocker is unchanged/known
      (Phase 12's `ForgeGameTestHooks` empty-annotation-scan issue) — per this task's brief, not
      re-attempted.
- [x] **Datagen propagation — done 2026-09-04 (12th session), without re-running `:neoforge:runData`
      (deliberately, to avoid Attempt 1's hang; the working tree already carried a fresh, verified-good
      `runData` output from an earlier session — the uncommitted `merge_potions.json` recipe + `.cache`
      entry).** Verified by unzipping the three production jars built in this session's
      `./gradlew build` (`{fabric,forge,neoforge}/build/libs/potionsplus-{fabric,forge,neoforge}-
      1.6.0.jar`) and counting, exactly Phase 10's method: blockstates 26/26/26, models 155/155/155,
      `data/potionsplus/tags/**` 11/11/11, `assets/potionsplus/sounds.json` 1/1/1,
      `data/potionsplus/recipe/**` 42/42/42 — all three jars identical. Zero `neoforge`-tagged leaks in
      the Fabric or Forge jars (`grep -ci neoforge` → 0 for both). The NeoForge jar's own
      `neoforge`-matching entries (120 hits) were inspected directly and are all expected/legitimate:
      `META-INF/neoforge.mods.toml`, `data/neoforge/loot_modifiers/...`,
      `data/potionsplus/neoforge/biome_modifier/...`, and `grill24/potionsplus/**/neoforge/*.class`
      (its own package-namespaced classes) — not resource leakage into another loader's jar.
- [x] **Production-jar smoke test — done 2026-09-04 (12th session), the `243ac95`-class check
      specifically, not just a dev-run repeat.** No `deploy-dev-builds` skill/script exists in this
      environment (grepped the repo and the available skill list — nothing found beyond this doc's own
      mention of it), and no GUI/profile-install path is available here, so this fell back to the
      packaged-jar-inspection approach the task brief allowed for. Built all three production jars
      (`{fabric,forge,neoforge}/build/libs/potionsplus-{fabric,forge,neoforge}-1.6.0.jar`, from this
      session's `./gradlew build`) and checked exactly the `243ac95` bug class: refmap/mixin-config
      presence, non-emptiness, and every mixin class actually resolving inside the jar.
      **Refmaps**: all three jars carry a non-empty `potionsplus-common-refmap.json` plus their own
      `potionsplus-{loader}-refmap.json`, each containing real `"mappings"` entries keyed by mixin
      class (spot-checked content, not just file size/presence).
      **Mixin-class resolution** (the actual `243ac95` failure mode — a mixin config listing a class
      that isn't really in the packaged jar): parsed all 6 mixin-config JSONs
      (`potionsplus.mixins.json` ×3 shared + `potionsplus.{fabric,forge,neoforge}.mixins.json`) with
      PowerShell's `ConvertFrom-Json`, built each package-qualified `.class` path from every
      `mixins`/`client`/`server` entry, and checked it against `System.IO.Compression.ZipFile`'s real
      entry list for that loader's jar — **zero missing classes across all three jars** (fabric: 8+10,
      forge: 6+10, neoforge: 8+10 mixin classes, all present).
      **Metadata wiring**: `fabric.mod.json`'s `"mixins"` array, NeoForge's `META-INF/neoforge.mods.toml`
      `[[mixins]]` blocks, and Forge's `META-INF/mods.toml` `[[mixins]]` blocks all correctly reference
      both their shared (`potionsplus.mixins.json`) and loader-specific config file names — read
      directly out of the packaged jars, not the source tree. This is the exact bug class `243ac95`
      fixed (a config referencing a mixin that production packaging dropped) and it does not reproduce
      here on any of the three loaders.
- [ ] **Manual smoke — deferred to a human with a real game client, exactly as this doc's standing
      caveat has treated live-gameplay verification through every prior phase (Phases 8/9/11 all
      closed with the identical "not verified in-world, no GUI-automation tool" note).** No
      GUI-automation tool exists in this environment; brewing a potion, drinking it, watching a
      loot-modifier fire, placing/using a clothesline, and confirming `LastPotionUsePlayerData`
      survives a respawn all require a human at the keyboard clicking through a real world. What *is*
      verified, cross-checked directly against the current code rather than assumed: the packet/BE/
      behaviour wiring these features depend on is identical across all three loaders (Phases 7-9
      already traced `ClotheslineBehaviour`, the loot-modifier classes, and
      `LastPotionUsePlayerData`'s `SavedData`-based persistence into `common/`, with per-loader
      registration only); this session's `runClient` smokes on all three loaders (see above) confirm
      the mod initializes, registers, and reaches a joined-world state without exceptions on the code
      paths these features sit on top of. The manual interaction step itself remains open — recorded
      honestly, not fabricated.

**Exit criterion: met, with the manual-smoke item explicitly deferred per this doc's established
pattern.** `./gradlew build` green (fresh `:common:test` 96/96). `:neoforge:runClient` clean
(carried over, evidence read directly). `:fabric:runClient` clean, reached a live joined world.
`:forge:runClient` client-boot clean; the world-load crash it hits is the pre-existing, already-
root-caused `SeededIngredientsLootTables` bug, not a regression. NeoForge and Fabric game tests both
re-confirmed 33/33; Forge's gametest blocker is unchanged and, per this phase's own brief, not
re-attempted. Datagen propagation confirmed via the packaged jars (matching resource counts, zero
cross-loader leaks) without needing to re-run `:neoforge:runData`. Production-jar mixin/refmap
smoke test done — the specific `243ac95` failure class checked and not reproduced on any of the
three loaders. Only the manual in-world interaction smoke (item 7) remains open, and it requires a
human with a real game client — **this phase is marked ✅ closed** on that basis, consistent with
how Phases 8/9/11 each closed with the same category of caveat.

**This closes the entire 13-phase multi-loader-expansion project.** Every phase P0-13 is now ✅
closed except Phase 9's status row text (superseded — see its row, effectively closed by Phase 11's
follow-on work) and Phase 12 (🟡 substantially done, Forge gametest's specific annotation-scan
blocker left open and documented, not a hard requirement per its own exit criterion). The two
categories of work that remain open project-wide are the same two named throughout: Forge's
`ForgeGameTestHooks` empty-scan gametest blocker (Phase 12), and all forms of live, human-driven
in-world/manual verification (every phase's standing "no GUI-automation tool" caveat, most
concretely Phase 13 item 7 here).
...
| 2026-09-04 | 11 | **Phase 11 CLOSED (10th session).** Re-verified the 8th session's "sole remaining item" (`ClientItemStacksTooltip`/`ClientTooltipComponentFactoriesListeners`) against current code and found it was already fixed by the 9th session's Phase 7 work — the Phase 11 status row and section text just hadn't been updated to say so. `common/utility/ClientItemStacksTooltip.java` has zero `net.neoforged`/`net.minecraftforge` imports; `event.{fabric,forge,neoforge}.ClientTooltipComponentFactoriesListeners` all register it. Read the entire Phase 11 section end-to-end for any other still-open item — found none. `:neoforge:build :fabric:build :forge:build -x test` → `BUILD SUCCESSFUL`; Decision 4a `comm -12` (exact doc formula, package-directory intersection) empty on all three. Updated the Status table row and added a closing paragraph to the Phase 11 section itself. Not independently re-verified: in-world rendering (still no GUI-automation tool, standing caveat). |
| 2026-09-04 | 12 | **Phase 12 — JUnit restored, NeoForge + Fabric game tests wired and green (33/33 each), Forge game tests real-progress-but-still-blocked (11th session).** Full writeup in the Phase 12 section itself; summary: moved the junit dependency block from `neoforge/build.gradle` (where Phase 1 stranded it) into `common/build.gradle` — `:common:test` → 87 tests, 0 failures, `PotionContentsAccessTest` passes. Found and fixed the exact `loom.mods`-gotcha this doc already flagged (confirmed empirically: it silently broke FML's annotation scan on NeoForge rather than crashing discovery) via the prescribed `compileTestmodJava`-destination-dir redirect; `:neoforge:runGametest` → 33/33. Refactored `common/src/testmod`'s two shared test files to be genuinely loader-agnostic (previously carried NeoForge-only annotations directly despite living in `common/` — a real misplacement bug) and moved the NeoForge-only `TestPlayers` out to `neoforge/`. Wired Fabric's own `fabric-gametest-api-v1` testmod (found and worked around a real API-shape divergence: this branch's pinned fabric-api version resolves a materially older gametest-api-v1, 2.0.5, than the reference tree's, with a different annotation shape entirely) — `:fabric:runGametest` → 33/33. Found and fixed a real content bug the tests caught along the way: 3 tests hardcoded `geode_grace`, a potion only ever registered on NeoForge (Phase 7's still-open `PotionsRegistrar` bucket) — switched to `MAGNETIC`, common to all loaders, re-verified both loaders green after. Forge: found and fixed three distinct, real build/launch infra bugs (missing `architectury-transformer-runtime`/`dev-launch-injector` jars on the gametest run specifically, then a JPMS split-package collision from a wrong `extendsFrom` target once those were fixed) that took `:forge:runGametest` from "cannot launch the JVM at all" to "boots fully through FML/Forge and reaches the exact same `ForgeGameTestHooks` checkpoint NeoForge used to fail at before its own fix" — but the final blocker (the ASM annotation scan itself finding zero test methods) was not resolved this session, despite the identical fix that resolved it for NeoForge already being in place for Forge too. This is a materially deeper blocker than the 26.1.2 reference tree's Forge ever hit (that tree's Forge never got past `RegisterEvent` not firing at all, on a different/newer gametest API generation) — a new problem, not a re-hit of an old one. Phase 12 marked 🟡 substantially done, not ✅ closed, because of this. Killed all game-test JVMs after each run; confirmed via `Get-CimInstance Win32_Process` no `architectury.main.class`/`TransformerRuntime` processes remained orphaned (headless gametest servers self-exit on their own; nothing needed manual killing this session). Not committed, per explicit instruction — changeset: `common/build.gradle` (junit block); `neoforge/build.gradle`, `fabric/build.gradle`, `forge/build.gradle` (testmod sourceSet wiring, gametest runs); `common/src/testmod/java/grill24/potionsplus/gametest/{AlchemyGameTests,BrewingCauldronGameTests}.java` (loader-agnostic refactor + the `geode_grace`→`MAGNETIC` fix); deleted `common/src/testmod/java/grill24/potionsplus/gametest/TestPlayers.java`; new `neoforge/src/testmod/java/grill24/potionsplus/neoforge/gametest/{NeoForgeTestPlayers,PotionsPlusNeoForgeGameTests}.java`; new `fabric/src/testmod/java/grill24/potionsplus/fabric/gametest/PotionsPlusFabricGameTests.java` + `fabric/src/testmod/resources/fabric.mod.json`; new `forge/src/testmod/java/grill24/potionsplus/forge/gametest/PotionsPlusForgeGameTests.java`; this doc — on `dev/1.21.1/multi-loader-expansion`, ready for review/commit. |
| 2026-09-04 | 13 | **Phase 13 done, closing the entire project (12th session, third same-day attempt at this exact
phase — the first two either hung on a `runClient`/hang-handling mistake or lost the agent process
mid-task; this session read both prior attempts' state before doing anything and did not repeat
either failure).** Carried over `:neoforge:runClient`'s already-clean evidence from the lost attempt
(read the leftover 628-line log's tail directly rather than re-claiming it, then deleted the scratch
file). `./gradlew build`: full green, then `:common:test --rerun-tasks` forced fresh (not
cache-trusted) — 96/96, 0 failures. `:fabric:runClient`: launched `run_in_background`, watched for
`Sound engine started`, it actually went further and joined a live dev world (JEI GUI, player join,
advancement toast) with only the known/documented `golden_cubensis`/`diamour` content-gap parse
errors (non-fatal, caught internally, Phase 7/12's already-named gap) — killed the JVM (pid 71940)
the instant the target state was confirmed, verified no orphans by process-scanning immediately
after. `:forge:runClient`: same procedure — client boot itself clean, but the integrated server then
hit the exact `SeededIngredientsLootTables.getItemsInTags` `NoSuchElementException` Phase 8's session
already found, root-caused, and confirmed pre-existing via `git stash` — read the actual crash report
file (`forge/run/crash-reports/crash-2026-09-04_21.36.55-server.txt`) to confirm the stack trace
matches rather than assuming from the log alone; the JVM exited on its own, no orphan-kill needed.
NeoForge and Fabric game tests both re-run headless (no game JVM involved, so no hang risk) — 33/33
on each, confirming Phase 12's result still holds. Datagen: checked the packaged jars from this
session's own build first, found matching resource counts (26 blockstates/155 models/11 tag files/1
sounds.json/42 recipes, identical across all three jars) and zero `neoforge`-tagged leaks in Fabric or
Forge — decided this was sufficient evidence and deliberately did **not** re-run `:neoforge:runData`
(the exact task that hung Attempt 1), consistent with the brief's explicit permission to skip it if
already-regenerated output proves sufficient. Production-jar smoke: no `deploy-dev-builds`
skill/script exists in this environment (checked the skill list and grepped the repo — only this
doc's own mention of it exists), so fell back to direct jar inspection — parsed all 6 packaged mixin
configs with PowerShell's `ConvertFrom-Json`, resolved every listed mixin class against each loader's
real `ZipFile` entry list via `System.IO.Compression.ZipFile`, confirmed zero missing classes across
fabric/forge/neoforge (this is precisely the `243ac95` failure class) — also confirmed all three
jars' refmaps are non-empty with real content and that `fabric.mod.json`/both `mods.toml`s correctly
reference their mixin config files by name. Manual smoke (item 7): deferred to a human with a real
client, per this doc's established pattern for every prior phase's identical caveat — not attempted,
not fabricated. Killed every game JVM this session launched; confirmed via repeated
`Get-CimInstance Win32_Process` scans that nothing was left orphaned at any point, including after the
Forge crash. **Phase 13 marked ✅ closed, closing the entire 13-phase project** — see the updated
Exit-criterion paragraph in the Phase 13 section for the precise final state and the two categories of
work still genuinely open (Forge's Phase-12 gametest blocker; all human-driven manual/in-world
verification). Not committed, per explicit instruction — changeset: this doc only (no source files
touched this session; the uncommitted `ClientCommands.java`/datagen files from earlier sessions are
still sitting in the working tree exactly as this session found them, verified-good, ready for
