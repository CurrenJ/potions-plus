...
public class LavaGeyserFeature extends Feature<NoneFeatureConfiguration> {
    ...
    public boolean tryPlace(FeaturePlaceContext<NoneFeatureConfiguration> context, BlockPos pos, Direction direction) {
        if (isValid(context, pos, direction)) {
            switch (direction) {
                case UP:
                    context.level().setBlock(pos, Blocks.LAVA_GEYSER.get().defaultBlockState().setValue(GeyserBlock.FACE, AttachFace.CEILING), 2);
                    break;
                case DOWN:
                    context.level().setBlock(pos, Blocks.LAVA_GEYSER.get().defaultBlockState().setValue(GeyserBlock.FACE, AttachFace.FLOOR), 2);
                    break;
                default:
                    context.level().setBlock(pos, Blocks.LAVA_GEYSER.get().defaultBlockState().setValue(GeyserBlock.FACE, AttachFace.WALL), 2);
                    context.level().setBlock(pos, Blocks.LAVA_GEYSER.get().defaultBlockState().setValue(BlockStateProperties.HORIZONTAL_FACING, direction.getOpposite()), 2);
                    break;
            }
