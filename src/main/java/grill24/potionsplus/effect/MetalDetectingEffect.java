...
public class MetalDetectingEffect extends MobEffect {
    ...
    public void applyEffectTick(LivingEntity livingEntity, int amplifier) {
        ...
        Set<ResourceLocation> detectedBlocks = new HashSet<>();
            ...
            for (int y = -range; y <= range; y++) {
                for (int z = -range; z <= range; z++) {
                    ...
                    if (DETECTED_BLOCKS.contains(block)) {
                        ...
                        detectedBlocks.add(block.getRegistryName());
                    }
                }
            }
        }
        ...
        if (!detectedBlockPos.isEmpty() && !livingEntity.level.isClientSide) {
            ...
            if (detectedBlocks.contains(Blocks.COPPER_ORE.getRegistryName()) || detectedBlocks.contains(Blocks.DEEPSLATE_COPPER_ORE.getRegistryName())) {
                livingEntity.level.playSound(null, soundPos, Sounds.PING_0.get(), SoundSource.BLOCKS, volume, 1.0F);
            }

            if (detectedBlocks.contains(Blocks.IRON_ORE.getRegistryName()) || detectedBlocks.contains(Blocks.DEEPSLATE_IRON_ORE.getRegistryName())) {
                livingEntity.level.playSound(null, soundPos, Sounds.PING_1.get(), SoundSource.BLOCKS, volume, 1.0F);
            }

            if (detectedBlocks.contains(Blocks.GOLD_ORE.getRegistryName()) ||
                    detectedBlocks.contains(Blocks.DEEPSLATE_GOLD_ORE.getRegistryName()) ||
                    detectedBlocks.contains(Blocks.NETHER_GOLD_ORE.getRegistryName()) ||
                    detectedBlocks.contains(Blocks.GILDED_BLACKSTONE.getRegistryName())) {
                livingEntity.level.playSound(null, soundPos, Sounds.PING_2.get(), SoundSource.BLOCKS, volume, 1.0F);

                if (detectedBlocks.contains(Blocks.ANCIENT_DEBRIS.getRegistryName())) {
