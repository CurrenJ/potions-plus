...
import grill24.potionsplus.utility.registration.block.SimpleCrossBlockModelGenerator;
...
public class FlowerBlocks {
    ...
    public static void init(BiFunction<String, Supplier<Block>, Holder<Block>> registerBlock, BiFunction<String, Supplier<Item>, Holder<Item>> registerItem) {
        LUNAR_BERRY_BUSH = RegistrationUtility.register(registerBlock, SimpleBlockBuilder.createSimple("lunar_berry_bush")
                .blockFactory(prop -> new LunarBerryBushBlock(prop.mapColor(MapColor.PLANT).noCollision().randomTicks().instabreak().sound(SoundType.SWEET_BERRY_BUSH).noOcclusion().lightLevel(LunarBerryBushBlock.LIGHT_EMISSION)))
        ...
        IRON_OXIDE_DAISY = RegistrationUtility.register(registerBlock, SimpleBlockBuilder.createSimple("iron_oxide_daisy")
                .blockFactory(prop -> new OreFlowerBlock(MobEffects.MAGNETIC, 200, prop.mapColor(MapColor.PLANT).noCollision().instabreak().sound(SoundType.GRASS), false, null,
                        state -> state.is(BlockTags.DIRT) || state.getBlock() instanceof FarmlandBlock,
                        state -> state.is(grill24.potionsplus.core.ConventionalTags.Blocks.ORES_IRON),
                        0.15f))
        ...
        COPPER_CHRYSANTHEMUM = RegistrationUtility.register(registerBlock, SimpleBlockBuilder.createSimple("copper_chrysanthemum")
                .blockFactory(prop -> new OreFlowerBlock(MobEffects.FORTUITOUS_FATE, 200, prop.mapColor(MapColor.PLANT).noCollision().instabreak().sound(SoundType.GRASS), false, null,
                        state -> state.is(BlockTags.DIRT) || state.getBlock() instanceof FarmlandBlock,
                        state -> state.is(grill24.potionsplus.core.ConventionalTags.Blocks.ORES_COPPER),
                        0.15f))
                .modelGenerator(SimpleCrossBlockModelGenerator::new)
                .renderType(BlockBuilder.RenderType.CUTOUT)
        ).getHolder();
        ...
        LAPIS_LILAC = RegistrationUtility.register(registerBlock, SimpleBlockBuilder.createSimple("lapis_lilac")
                .blockFactory(prop -> new OreFlowerBlock(MobEffects.LOOTING, 200, prop.mapColor(MapColor.PLANT).noCollision().instabreak().sound(SoundType.GRASS), false, null,
                        state -> state.is(BlockTags.DIRT) || state.getBlock() instanceof FarmlandBlock,
                        state -> state.is(grill24.potionsplus.core.ConventionalTags.Blocks.ORES_LAPIS),
                        0.3f))
                .modelGenerator(SimpleCrossBlockModelGenerator::new)
                .renderType(BlockBuilder.RenderType.CUTOUT)
        ).getHolder();
        ...
        DIAMOUR = RegistrationUtility.register(registerBlock, SimpleBlockBuilder.createSimple("diamour")
                .blockFactory(prop -> new OreFlowerBlock(MobEffects.TELEPORTATION, 200, prop.mapColor(MapColor.PLANT).noCollision().instabreak().sound(SoundType.GRASS), false, null,
                        state -> state.is(BlockTags.DIRT) || state.getBlock() instanceof FarmlandBlock,
                        state -> state.is(grill24.potionsplus.core.ConventionalTags.Blocks.ORES_DIAMOND),
                        0.15f))
                .modelGenerator(SimpleCrossBlockModelGenerator::new)
                .renderType(BlockBuilder.RenderType.CUTOUT)
        ).getHolder();
        ...
        GOLDEN_CUBENSIS = RegistrationUtility.register(registerBlock, SimpleBlockBuilder.createSimple("golden_cubensis")
                .blockFactory(prop -> new OreFlowerBlock(MobEffects.GEODE_GRACE, 200, prop.mapColor(MapColor.PLANT).noCollision().instabreak().sound(SoundType.GRASS), false, null,
                        state -> state.is(BlockTags.DIRT) || state.getBlock() instanceof FarmlandBlock,
                        state -> state.is(grill24.potionsplus.core.ConventionalTags.Blocks.ORES_GOLD),
                        0.2f))
                .modelGenerator(SimpleCrossBlockModelGenerator::new)
                .renderType(BlockBuilder.RenderType.CUTOUT)
        ).getHolder();
        ...
        REDSTONE_ROSE = RegistrationUtility.register(registerBlock, SimpleBlockBuilder.createSimple("redstone_rose")
                .blockFactory(prop -> new OreFlowerBlock(MobEffects.REACH_FOR_THE_STARS, 200, prop.mapColor(MapColor.PLANT).noCollision().instabreak().sound(SoundType.GRASS), false, null,
                        state -> state.is(BlockTags.DIRT) || state.getBlock() instanceof FarmlandBlock,
                        state -> state.is(grill24.potionsplus.core.ConventionalTags.Blocks.ORES_REDSTONE),
        ...
        BLACK_COALLA_LILY = RegistrationUtility.register(registerBlock, SimpleBlockBuilder.createSimple("black_coalla_lily")
                .blockFactory(prop -> new OreFlowerBlock(net.minecraft.world.effect.MobEffects.FIRE_RESISTANCE, 200, prop.mapColor(MapColor.PLANT).noCollision().instabreak().sound(SoundType.GRASS), false, null,
                        state -> state.is(BlockTags.DIRT) || state.getBlock() instanceof FarmlandBlock,
                        state -> state.is(grill24.potionsplus.core.ConventionalTags.Blocks.ORES_COAL),
                        0.1f))
                .modelGenerator(SimpleCrossBlockModelGenerator::new)
                .renderType(BlockBuilder.RenderType.CUTOUT)
        ).getHolder();
