...
public class OreItems {
    ...
    public static void init(BiFunction<String, Supplier<Item>, Holder<Item>> register) {
        NETHERITE_REMNANT = RegistrationUtility.register(register, SimpleItemBuilder.createSimple("netherite_remnant")
                ...
                .recipeGenerator(holder -> new RecipeGeneratorUtility.RecipeGenerator<>(holder, ppId("remnant_debris_to_netherite_remnant_smelting"),
                        (recipeProvider, h) ->
                                SimpleCookingRecipeBuilder.smelting(
                                        recipeProvider.tag(Tags.Items.REMNANT_DEBRIS),
                                        RecipeCategory.MISC,
                                        new ItemStack(h.value()),
                                        2.0F,
                                        100
                                ).unlockedBy("has_remnant_debris", recipeProvider.has(OreBlocks.REMNANT_DEBRIS.value()))))
                .recipeGenerator(holder -> new RecipeGeneratorUtility.RecipeGenerator<>(holder, ppId("remnant_debris_to_netherite_remnant_blasting"),
                        (recipeProvider, h) ->
                ...
                .recipeGenerator(holder -> new RecipeGeneratorUtility.RecipeGenerator<>(holder, ppId("netherite_ingot"),
                        (recipeProvider, h) ->
                                recipeProvider.shapeless(RecipeCategory.MISC, net.minecraft.world.item.Items.NETHERITE_INGOT)
                                        .requires(net.minecraft.world.item.Items.NETHERITE_SCRAP, 2)
                                        .requires(h.value(), 1)
                                        .requires(net.minecraft.world.item.Items.GOLD_INGOT, 4)
                                        .group("netherite_ingot")
                                        .unlockedBy("has_netherite_remnant", recipeProvider.has(h.value()))))
        ).getHolder();
