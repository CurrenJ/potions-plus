...
public class LeashRenderer {
    ...
    private static Vector3f addVertexPair(VertexConsumer vertexConsumer, Matrix4f matrix, float deltaX, float deltaY, float deltaZ, int blockLightStart, int blockLightEnd, int skyLightStart, int skyLightEnd, float width, float height, float deltaZInvDistance, float deltaXInvDistance, int step, boolean reverse) {
        ...
        if (vertexConsumer != null && matrix != null) {
            vertexConsumer.addVertex(matrix, interpolatedX - deltaZInvDistance, interpolatedY + height, interpolatedZ + deltaXInvDistance).setColor(red, green, blue, 1.0F).setLight(packedLight);
            vertexConsumer.addVertex(matrix, interpolatedX + deltaZInvDistance, interpolatedY + width - height, interpolatedZ - deltaXInvDistance).setColor(red, green, blue, 1.0F).setLight(packedLight);
        }
    ...
    private static void addVertices(VertexConsumer vertexConsumer, Matrix4f matrix, Vector3f[] points, int blockLightStart, int blockLightEnd, int skyLightStart, int skyLightEnd, float width, float height, float deltaZInvDistance, float deltaXInvDistance, boolean reverse) {
        for (int i = 0; i < points.length; i++) {
            ...
            if (vertexConsumer != null && matrix != null) {
                vertexConsumer.addVertex(matrix, point.x() - deltaZInvDistance, point.y() + height, point.z() + deltaXInvDistance).setColor(red, green, blue, 1.0F).setLight(packedLight);
                vertexConsumer.addVertex(matrix, point.x() + deltaZInvDistance, point.y() + width - height, point.z() - deltaXInvDistance).setColor(red, green, blue, 1.0F).setLight(packedLight);
            }
        }
    }
    ...
    public static void renderLeashPoints(Vector3f[] leashPoints, PoseStack poseStack, MultiBufferSource bufferSource, int blockLightStart, int blockLightEnd, int skyLightStart, int skyLightEnd) {
        ...
        for (int i = 0; i <= 24; ++i) {
            ...
            vertexConsumer.addVertex(matrix, point.x(), point.y(), point.z()).setColor(0.5F, 0.4F, 0.3F, 1.0F).setLight(LightTexture.pack(blockLightStart, skyLightStart));
        }

        for (int i = 24; i >= 0; --i) {
            Vector3f point = leashPoints[i];
            vertexConsumer.addVertex(matrix, point.x(), point.y(), point.z()).setColor(0.5F, 0.4F, 0.3F, 1.0F).setLight(LightTexture.pack(blockLightEnd, skyLightEnd));
        }
    }
}