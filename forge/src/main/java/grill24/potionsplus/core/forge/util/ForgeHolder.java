 ...
 *
 * <p>1.21.1 delta vs the 26.1.2 template: {@code Holder<T>} here already extends
 * {@code Supplier<T>}, so {@code implements Supplier<T>} is dropped, and this MC version's
 * {@code Holder} has no {@code components()}/{@code areComponentsBound()} to override.
 */
public final class ForgeHolder<T> implements Holder<T> {
     ...
     * the registry directly. Matching by key here mirrors {@link #is(Holder)}'s existing semantics, at
     * the cost of one-sided symmetry - the vanilla reference's own identity-based equals still won't
     * recognise this wrapper - which is an accepted, unavoidable tradeoff without patching vanilla
     * (see {@code RegistryMixin} for the same tradeoff made for serialization).
     */
    @Override
