 ...
 *
 * <p>Loader-agnostic on purpose (Phase 12): these are plain static methods with no gametest-framework
 * annotations at all - each loader's own {@code gametest} testmod source set carries a thin wrapper
 * class with that loader's own {@code @GameTest} annotation on a method that just delegates here, e.g.
 * {@code neoforge.gametest.PotionsPlusNeoForgeGameTests}/{@code fabric.gametest.
 * PotionsPlusFabricGameTests}. NeoForge's {@code @GameTestHolder}/{@code @PrefixGameTestTemplate} used
 * to sit directly on this class, which meant it could only ever be compiled - and discovered - by
 * NeoForge's own annotation scan; Fabric and Forge need their own discovery mechanisms entirely (see
 * docs/multi-loader-expansion.md Phase 12).
 */
public final class AlchemyGameTests {
    ...
    public static void modPotionsRoundTripThroughEveryContainer(GameTestHelper helper) {
        // MAGNETIC_POTIONS, not the neoforge-only PotionsRegistrar.GEODE_GRACE_POTIONS: geode_grace's
        // *potion* (not the raw effect) is only generated by core.neoforge.potion.PotionsRegistrar
        // (Phase 7's still-open "8 potions with @EventBusSubscriber-coupled effects" bucket). Confirmed
        // empirically this session - a first attempt at this test that looked geode_grace up by name
        // in Potions.ALL_POTION_GENERATION_DATA (rather than importing the neoforge-only field
        // directly) still failed on :fabric:runGametest with "geode_grace potion was not generated",
        // because the entry itself is never added to that list on Fabric/Forge, not just harder to
        // import. MAGNETIC_POTIONS is common/ (core.potion.Potions), so it generates identically on
        // every loader - same fix applied to BrewingCauldronGameTests' two GEODE_GRACE-keyed tests.
