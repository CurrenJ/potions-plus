...
public class PotionUpgradeIngredients implements IPotionUpgradeIngredients {
    private static final Logger LOGGER = LoggerFactory.getLogger(PotionUpgradeIngredients.class);
    public enum Rarity {
        NONE,
        COMMON,
        RARE;
    ...
    private static PpMultiIngredient sampleUniqueIngredientsFromSamplingConfig(Map<Rarity, IngredientSamplingConfig> config, Set<PpMultiIngredient> allPreviouslyGeneratedRecipeInputs, PpIngredient inputPotion, Holder<Potion> outputPotion, RandomSource randomSource) {
        ...
        do {
            if (ppMultiIngredient != null && attempts > 1 && Debug.DEBUG && Debug.DEBUG_POTION_INGREDIENTS_GENERATION) {
                LOGGER.info("[BCR] Regenerating ingredients for recipe due to collision: " + ppMultiIngredient);
            }
            ...
            if (ingredients.isEmpty()) {
                LOGGER.warn("No ingredients were sampled for potion " + outputPotion.unwrapKey().map(k -> k.identifier().toString()).orElse("[unregistered]"));
                throw new IllegalStateException("No ingredients were sampled for potion " + outputPotion.unwrapKey().map(k -> k.identifier().toString()).orElse("[unregistered]"));
            }
