...
public class OreItems {
    ...
    public static void init(BiFunction<String, Supplier<Item>, Holder<Item>> register) {
        RAW_URANIUM = RegistrationUtility.register(register, SimpleItemBuilder.createSimple("raw_uranium").properties(new Item.Properties().rarity(Rarity.UNCOMMON))
        ...
        URANIUM_INGOT = RegistrationUtility.register(register, SimpleItemBuilder.createSimple("uranium_ingot").properties(new Item.Properties().rarity(Rarity.UNCOMMON))
                .recipeGenerator(holder -> new RecipeGeneratorUtility.RecipeGenerator<>(holder, ppId("uranium_ore_to_uranium_ingot_smelting"),
                        (recipeProvider, h) ->
                                SimpleCookingRecipeBuilder.smelting(
                                        Ingredient.of(recipeProvider.getRegistries().lookupOrThrow(Registries.ITEM).getOrThrow(Tags.Items.URANIUM_ORE)),
                                        RecipeCategory.MISC,
                                        CookingBookCategory.MISC,
                                        h.value(),
                                        2.0F,
