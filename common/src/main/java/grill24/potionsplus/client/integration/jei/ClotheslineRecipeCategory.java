...
public class ClotheslineRecipeCategory implements IRecipeCategory<ClotheslineRecipe> {
    ...
    @Override
    public void draw(ClotheslineRecipe recipe, IRecipeSlotsView recipeSlotsView, GuiGraphicsExtractor guiGraphics, double mouseX, double mouseY) {
        ...
        if (successChance < 1.0f) {
            ...
            Component successText = Component.literal(percentage + "% Success:");
            ...
            if (!recipe.getFallbackResult().isEmpty()) {
                Component fallbackText = Component.literal("Otherwise:");
