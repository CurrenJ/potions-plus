...
// same JVM, and both call tick() here (see each loader's tick listeners) while other code
// concurrently calls queueDelayedEvent() from either thread. A plain ArrayList's removeIf() is not
// safe under concurrent mutation and throws ConcurrentModificationException under that access
// pattern; CopyOnWriteArrayList tolerates it.
