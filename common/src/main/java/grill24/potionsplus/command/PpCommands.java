...
/**
 * Loader-agnostic debug command tree, registered by each platform's command-registration hook
 * (only ever wired up when {@link Debug#DEBUG} is true).
 */
                ...
                .then(Commands.literal("potionHand")
                        ...
                        .executes(context -> {
                            if (context.getSource().getEntity() instanceof ServerPlayer player) {
                                ...
                                player.setItemInHand(InteractionHand.MAIN_HAND, withPotion);
                            }
                            return 1;
                        })
                )
